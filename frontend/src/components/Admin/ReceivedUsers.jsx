...
const ReceivedUsers = () => {
  ...
  // Filter users to display only those where the current tab's admin is in received_admins
  const filteredUsers = users.filter((user) => {
    const receivedAdmins = user.received_admins ? JSON.parse(user.received_admins) : [];
    return receivedAdmins.includes(currentTabAdmin);
  });
    ...
    <>
      ...
      <div className="container mx-auto lg:px-4 px-2 py-16 mb-16 lg:block hidden">
        <h1 className="text-4xl font-bold text-[#0051FF] text-center mb-8">
          Received Users {currentTabAdmin && `- ${formatAdminName(currentTabAdmin)}`}
        </h1>
      ...
      <div className="container mx-auto px-4 py-8 mb-20 lg:hidden block">
        ...
        <div className="space-y-4">
          {currentUsers.map((user, index) => {
            ...
            const receivedAdmins = user.received_admins ? JSON.parse(user.received_admins) : [];
            const receivedFromAdmins = receivedAdmins.includes(currentTabAdmin)
              ? forwardedAdmins
              : [];
              ...
              >
                ...
                <div className="text-sm text-gray-900">
                  ...
                  <div className="flex flex-wrap gap-2 mt-1">
                    {receivedFromAdmins.map((admin, adminIndex) => (
                      <span
                        key={adminIndex}
...
const ReceivedUserRow = ({ user, currentTabAdmin, handleViewInfo }) => {
  // Parse forwarded_admins and received_admins from the user object
  const forwardedAdmins = user.forwarded_admins ? JSON.parse(user.forwarded_admins) : [];
  const receivedAdmins = user.received_admins ? JSON.parse(user.received_admins) : [];

  // Display the forwarded_admins (admins who forwarded the form to the current tab's admin)
  const receivedFromAdmins = receivedAdmins.includes(currentTabAdmin)
    ? forwardedAdmins
    : [];
    ...
    <tr className="hover:bg-gray-50 text-center">
      ...
      <td className="px-6 py-4 text-sm text-gray-900 border-r">
        <div className="flex flex-wrap gap-2 justify-center">
          {receivedFromAdmins.map((admin, index) => (
            <span
              key={index}
              className="bg-blue-100 text-blue-800 text-sm font-medium px-2.5 py-0.5 rounded-full"
            >
