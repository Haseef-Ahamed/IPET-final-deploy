...
const AcademicQualification = () => {
  ...
  // Parse user data
  const subjects = userData ? JSON.parse(userData.subjects || "[]") : [];
  const credits = userData ? JSON.parse(userData.credits || "[]") : [];
  const higherEducationInstitutes = userData ? JSON.parse(userData.higher_education_institutes || "[]") : [];
          ...
          <div className="lg:bg-[#EDEDED] lg:p-6 p-4 rounded-lg lg:shadow-sm">
            <div className="bg-white p-4 sm:p-6 rounded-lg shadow-sm">
              ...
              <div className="mb-8 p-4 sm:p-6 bg-gray-50 rounded-lg border border-gray-200">
                <h3 className="text-[16px] font-[600] mb-4 text-gray-700">
                  GCE Advanced Level
                </h3>
                {userData?.examination || userData?.year || subjects.length > 0 ? (
                      ...
                      {subjects.length > 0 ? (
                        <div className="space-y-2">
                          {subjects.map((subject, index) => (
                            <div
                              key={index}
                              className="flex justify-between p-2 bg-white border border-gray-300 rounded-md"
                            >
                              <span className="text-[14px] sm:text-[16px] text-gray-900">
                                {subject || "N/A"}
                              </span>
                              <span className="text-[14px] sm:text-[16px] text-gray-900">
                                {credits[index] || "N/A"}
                        ...
                        <p className="text-[14px] sm:text-[16px] text-gray-600">
                          No subjects or credits available.
                        </p>
                      )}
                    </div>
                    {userData?.al_certificate_path && (
                      <div className="sm:col-span-2">
                        <label className="text-[16px] font-[600] block mb-1 text-gray-700">
                          A/L Certificate
                        </label>
                        <button
                          onClick={() => handleDownloadCertificate(userData?.al_certificate_path)}
              ...
              <div className="p-4 sm:p-6 bg-gray-50 rounded-lg border border-gray-200">
                ...
                {higherEducationInstitutes.length > 0 ? (
                    ...
                    <div key={index} className="mb-6 last:mb-0">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="text-[14px] sm:text-[16px] text-gray-900">
                            {institute.periodOfStudyFrom || "N/A"}
                          </p>
                        </div>
                        <div>
                          <label className="text-[16px] font-[600] block mb-1 text-gray-700">
                        ...
                        <div className="sm:col-span-2">
                          ...
                          {institute.qualifications?.length > 0 ? (
                            institute.qualifications.map((qualification, qIndex) => (
                              <p
                                key={qIndex}
                                className="text-[14px] sm:text-[16px] text-gray-900 p-2 bg-white border border-gray-300 rounded-md mb-2 last:mb-0"
                              >
                        ...
                        {institute.certificatePath && (
                          <div className="sm:col-span-2">
                            ...
                            <button
                              onClick={() => handleDownloadCertificate(institute.certificatePath)}
                              className="w-full sm:w-auto bg-[#2A3990] text-[14px] sm:text-[16px] font-[600] text-white px-4 py-2 rounded-md hover:bg-[#1b2142] transition-colors focus:outline-none focus:ring-2 focus:ring-[#2A3990] focus:ring-opacity-50"
                            >
                              Download Certificate
                            </button>
                  ...
                  <p className="text-[16px] text-gray-600 text-center">
                    No higher education data available.
                  </p>
                )}
              </div>
