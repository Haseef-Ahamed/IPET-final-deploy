...
const ApprovalSection = () => {
  ...
  // const handleForward = () => {
  //   if (userData) {
  //     // Navigate to RegisteredUsers with the approved user data
  //     navigate("/register-users", { 
  //       state: { 
  //         approvedUser: {
  //           ...userData,
  //           date: new Date().toISOString().split('T')[0].replace(/-/g, '/') // Add current date
  //         }
  //       } 
  //     });
  //   }
  ...
  const handleForward = () => {
    if (userData) {
      ...
      const existingUsers = JSON.parse(localStorage.getItem('approvedUsers') || '[]');
      
      // Check if user already exists
      const userExists = existingUsers.some(user => user.idNo === approvedUser.idNo);
      
      if (!userExists) {
        // Add new user to the beginning of the array
        const updatedUsers = [approvedUser, ...existingUsers];
  ...
  const handleReject = () => {
    if (userData) {
      ...
      // Get existing approved users from localStorage
      const existingUsers = JSON.parse(localStorage.getItem('rejectedUsers') || '[]');
      
      // Check if user already exists
      const userExists = existingUsers.some(user => user.idNo === rejectedUser.idNo);
      
      if (!userExists) {
        // Add new user to the beginning of the array
        const updatedUsers = [rejectedUser, ...existingUsers];
        
        // Save to localStorage
        localStorage.setItem('rejectedUsers', JSON.stringify(updatedUsers));
      }
      
      // Navigate to RegisteredUsers
      navigate('/decline-users');
    }
  };
      ...
      <div className="bg-[#D9D9D9]  lg:py-2 py-6 lg:px-16 px-6">
        ...
        <div className="max-w-[1500px] mx-auto bg-white  shadow-md p-6 space-y-8 mb-28">
          ...
          <div className="flex flex-row sm:flex-row gap-4">
            ...
            <button onClick={handleReject} className="w-full sm:w-[227px] bg-[#b91c1c] text-[18px] font-[700] text-white py-2 px-4 rounded-md hover:bg-[#991818] transition-colors">
              Reject
            </button>
          </div>
