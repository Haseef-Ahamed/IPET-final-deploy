...
const ProfessionalMemberships = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        setFormData({
          memberships: userData.professional_memberships?.length
            ? userData.professional_memberships
            : [
                {
                  institution: "",
                  membershipNumber: "",
  ...
  const handleFileChange = async (e, index) => {
    ...
    const sanitizedFileName = file.name.replace(/%/g, "").replace(/[^a-zA-Z0-9.-]/g, "_");
    ...
    try {
      ...
      const newMemberships = [...formData.memberships];
      newMemberships[index].certificatePath = filePath;
      setFormData({ ...formData, memberships: newMemberships });
  ...
  const validateForm = () => {
    ...
    formData.memberships.forEach((membership, index) => {
      ...
      if (!membership.membershipNumber) {
        newErrors[`membershipNumber-${index}`] = "Membership number is required.";
      }
      if (!membership.joinedYear) {
        newErrors[`joinedYear-${index}`] = "Joined year is required.";
      }
      if (!membership.certificatePath) {
        newErrors[`certificate-${index}`] = "Membership certificate is required.";
      }
    });
    setErrors(newErrors);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const membershipData = {
        userId: userId,
        professionalMemberships: formData.memberships,
      };
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || "Failed to update membership data.");
      }

      const result = await response.json();
      console.log("Membership data updated successfully:", result.message);
      Swal.fire({
        icon: "success",
        title: "Success",
        text: "Membership data updated successfully!",
        confirmButtonColor: "#2A3990",
      });
    } catch (error) {
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: error.message || "Failed to update membership data. Please try again.",
        confirmButtonColor: "#2A3990",
      });
    }
  };
            ...
            {formData.memberships.map((membership, index) => (
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => handleInputChange(index, "joinedYear", e.target.value)}
                      className="w-full border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-[#2A3990]"
                      required
                    >
                  ...
                  <div>
                    ...
                    {membership.certificatePath && (
                      <div className="mt-2 text-[14px] text-gray-600">
                        ...
                        <button
                          type="button"
                          onClick={(e) => handleFileDownload(e, membership.certificatePath)}
                          className="text-[#2A3990] hover:underline cursor-pointer"
                        >
                          {decodeURIComponent(membership.certificatePath.split("/").pop())}
                        </button>
                      </div>
                    )}
                ...
                <div className="flex items-center justify-end gap-4 mt-6">
                  ...
                  {index === formData.memberships.length - 1 && (
                    <button
                      type="button"
                      onClick={addNewEntry}
                      className="px-3 py-2 rounded-md bg-[#2A3990] text-white hover:bg-[#1b2142] transition-colors"
