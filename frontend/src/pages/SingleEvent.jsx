...
const EventDetails = () => {
  ...
  useEffect(() => {
    const fetchEventDetails = async () => {
      try {
        const response = await fetch(`http://localhost:5000/api/events-courses/event/${eventId}`);
  ...
  const nextSlide = () => {
    setCurrentSlide(prev => prev === (eventData?.images?.length || 0) - 1 ? 0 : prev + 1);
  };

  const prevSlide = () => {
    setCurrentSlide(prev => prev === 0 ? (eventData?.images?.length || 0) - 1 : prev - 1);
  };
  ...
  if (!eventData) return <div className="flex items-center justify-center w-full h-screen">No event data found</div>;
    ...
    <>
      ...
      <div className="hidden md:block mb-24">
        ...
        <motion.div className="relative w-full h-[300px] md:h-[269px]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
          <div className="absolute inset-0">
            ...
            <img src={about_bg} alt="Events Hero" className="object-cover w-full h-full" />
          </div>
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between mb-4">
                <motion.span variants={itemVariants} className="text-[#2D387D] text-sm font-semibold">
                  {new Date(eventData.uploaded_date).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}
                </motion.span>
                <motion.button variants={itemVariants} className="text-sm font-semibold text-black-700 hover:underline" onClick={() => navigate("/view-events")}>
                  ← Go Back
                </motion.button>
              </div>
              <motion.h2 variants={itemVariants} className="text-2xl font-semibold text-[#2D387D] mb-4">{eventData.title}</motion.h2>
              <motion.div variants={itemVariants} className="mb-6">
                <motion.img src={eventData.main_image_url} alt={eventData.title} className="object-cover w-full rounded-lg h-96" whileHover={{ scale: 1.02 }} transition={{ duration: 0.3 }} />
              </motion.div>
            ...
            <div className="max-w-4xl pt-20 pb-40 mx-auto mt-10">
              <motion.div className="relative -mb-6 overflow-hidden" variants={itemVariants}>
                <div className="relative w-full h-[500px] mb-4">
                  <motion.img key={currentSlide} src={eventData.images[currentSlide]} alt={`Slide ${currentSlide + 1}`} className="w-full h-[500px] object-contain mx-auto" initial="hidden" animate="visible" variants={slideVariants} />
      ...
      <div className="w-full md:hidden mb-32">
        ...
        <motion.div className="relative w-full h-[169px]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
          <div className="absolute inset-0">
            <div className="absolute inset-0 bg-black/60 z-10" />
            <img src={about_bg_m} alt="Events Hero" className="object-cover w-full h-full" />
          </div>
          <div className="relative z-20 h-full flex items-center justify-center">
            <motion.h1 initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.6 }} className="text-[30px] font-[600] text-white">Events</motion.h1>
          </div>
        </motion.div>
        ...
        <motion.div initial="hidden" animate="visible" variants={containerVariants} className="w-full px-4 mt-6">
          ...
          <motion.div className="flex items-center justify-between mb-4" variants={itemVariants}>
            <span className="text-[#2543B1] text-sm font-semibold">{new Date(eventData.uploaded_date).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</span>
            <button className="text-sm font-semibold text-black-700 hover:underline" onClick={() => navigate("/view-events")}>← Go Back</button>
          </motion.div>
          <motion.h2 className="text-[16px] font-semibold text-[#2543B1] mb-4 text-center" variants={itemVariants}>{eventData.title}</motion.h2>
          <motion.div className="flex justify-center mb-4" variants={itemVariants}>
            <motion.img src={eventData.main_image_url} alt={eventData.title} className="object-cover w-full h-64" whileTap={{ scale: 0.98 }} />
          </motion.div>
            ...
            <motion.div className="w-full bg-[#E9ECF7] mt-10" variants={containerVariants}>
              <div className="w-full py-8">
                <div className="relative w-full h-[250px] mb-4">
                  <motion.img key={currentSlide} src={eventData.images[currentSlide]} alt={`Slide ${currentSlide + 1}`} className="w-full h-[250px] object-contain mx-auto" initial="hidden" animate="visible" variants={slideVariants} />
                  {eventData.images.length > 1 && (
                    <>
